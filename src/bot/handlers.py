...
async def reaction_handler(reaction: MessageReactionUpdated):
    ...
    if not triggered_text:
        # Если не нашли сообщение в истории (слишком старое), сообщаем пользователю.
        try:
            await reaction.bot.send_message(
                ...
                "Попробуйте на чем-нибудь новеньком!",
                parse_mode='HTML'
            )
...
async def command_help_handler(message: Message):
    ...
    triggers_str = ", ".join(MEME_TRIGGERS)
    help_text = (
        ...
        "2. <b>Контекст</b>: Я прочитаю диалог, чтобы мем был в тему.\n"
        "3. <b>Магия</b>: Подождите пару секунд, пока я рисую!\n\n"
...
async def command_start_handler(message: Message):
    ...
    triggers_str = " или ".join(MEME_TRIGGERS)
    welcome_text = (
        f"👋 <b>Привет! Я бот-мемогенератор.</b>\n\n"
        f"Добавь меня в чат и поставь реакцию {triggers_str} на любое сообщение, чтобы создать мем.\n\n"
